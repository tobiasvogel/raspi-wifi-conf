...
void MainWindow::unhideUserPassword() {
   if ( !userPasswordIsHidden() ) { return; }

   ui->userPasswordLineEdit->setEchoMode( QLineEdit::Normal );
   ui->hideUserPasswordPushButton->setIcon( QIcon( ":/hide-icon" ) );
}

void MainWindow::hideUserPassword() {
   if ( userPasswordIsHidden() ) { return; }

   ui->userPasswordLineEdit->setEchoMode( QLineEdit::Password );
   ui->hideUserPasswordPushButton->setIcon( QIcon( ":/unhide-icon" ) );
}

bool MainWindow::userPasswordIsHidden() {
   if ( ui->userPasswordLineEdit->echoMode() == QLineEdit::Password ) {
      return true;
...
void MainWindow::toggleUserPasswordVisibility() {
   if ( userPasswordIsHidden() ) {
      unhideUserPassword();
...
void MainWindow::generateOutput( void ) {
   ...
   if ( !ui->appendCheckBox->isChecked() && fileExists ) {
      ...
      if ( !configFile.open( QIODevice::Append ) ) {
         ...
         msgBox.setText(
            tr( "You can still Manually Copy the Generated Output to a File called \" wpa_supplicant.conf \".\nWould you like me to Display the generated File-Contents in order for you to Copy it?" ) );
...
void MainWindow::showAbout() {
   ...
   detailedText.append( "\n\n" );
   detailedText.append(
      tr( "Contributions and Translations welcome! Please submit Pull Requests via GitHub at %1" ).arg( "https://github.com/tobiasvogel/raspi-wifi-conf/pulls" ) );
